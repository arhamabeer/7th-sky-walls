...
| `test:interaction` | 348 behavioural checks across thirteen contexts — mobile, tablet, desktop, the Android AR tiers, AR launch-failure recovery, custom wording, Urdu and Arabic, the configurator brief, the client error sink, the materials page, the hanging height, the camera preview's calibration, reduced motion and the rate limiter. |
